...
#more refined volcano plot function used to plot and label a list of genes.
...
volcano.plot<-function(rnaseqdata, alphavalue = 0.05, lfcLim = 1.5, top = 10, genesetFile = NULL, graphname = "volcano",
                       ...
                       boxpadding = 0.5, labelRepel = 10, labelSize = 5, axisLabelsize = 16, axisTitle = 18) {
  ...
  v + geom_point(aes(col = sig, shape=shape)) + #add points colored by significance
    ...
    theme(legend.position = "none", axis.text = element_text(size=axisFontsize), axis.title = element_text(size=axisTitle,face="bold")) + #ggtitle("Volcanoplot")
